...
function buildpro_post_desc_add_meta_box($post_type, $post)
{
    ...
    add_meta_box('buildpro_post_tab_desc', esc_html__('Description', 'buildpro'), 'buildpro_post_desc_render_meta_box', 'post', 'normal', 'default');
}
add_action('add_meta_boxes', 'buildpro_post_desc_add_meta_box', 10, 2);

function buildpro_post_desc_render_meta_box($post)
{
    $post_desc_short = get_post_meta($post->ID, 'buildpro_post_description_short', true);
    echo '<style>
    ...
    echo '<p class="buildpro-post-field"><label>' . esc_html__('Description Short', 'buildpro') . '</label><textarea name="buildpro_post_description_short" rows="5" class="large-text">' . esc_textarea($post_desc_short) . '</textarea></p>';
    echo '</div>';
}
