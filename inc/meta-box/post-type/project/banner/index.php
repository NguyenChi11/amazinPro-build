...
function buildpro_project_banner_add_meta_box($post_type, $post)
{
    ...
    add_meta_box('buildpro_project_tab_banner', esc_html__('Banner Image', 'buildpro'), 'buildpro_project_banner_render_meta_box', 'project', 'normal', 'default');
}
...
function buildpro_project_banner_render_meta_box($post)
{
    ...
    echo '<p class="buildpro-post-field"><label>' . esc_html__('Banner Image', 'buildpro') . '</label><input type="hidden" id="project_banner_id" name="project_banner_id" value="' . esc_attr($banner_id) . '"> <button type="button" class="button" id="project_select_banner">' . esc_html__('Select photo', 'buildpro') . '</button> <button type="button" class="button" id="project_remove_banner">' . esc_html__('Remove photo', 'buildpro') . '</button></p>';
    echo '<div class="image-banner-preview" id="project_banner_preview">' . ($thumb ? '<img src="' . esc_url($thumb) . '">' : '<span style="color:#888">' . esc_html__('No banner selected', 'buildpro') . '</span>') . '</div>';
    echo '</div>';
    echo '<script>
    (function(){
        var i18n = ' . $i18n . ';
        ...
        if(selectBtn){
            selectBtn.addEventListener("click", function(e){
                e.preventDefault();
                if(!frame){ frame = wp.media({ title: i18n.selectImage, button: { text: i18n.add }, multiple: false, library: { type: "image" } }); }
        ...
        if(removeBtn){
            removeBtn.addEventListener("click", function(e){
                e.preventDefault();
                input.value = "";
                preview.innerHTML = "<span style=\\"color:#888\\">"+i18n.empty+"</span>";
            });
        }
    })();
    </script>';
