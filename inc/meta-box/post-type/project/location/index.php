...
function buildpro_project_location_add_meta_box($post_type, $post)
{
    ...
    add_meta_box('buildpro_project_tab_location', esc_html__('Location', 'buildpro'), 'buildpro_project_location_render_meta_box', 'project', 'normal', 'default');
}
...
function buildpro_project_location_render_meta_box($post)
{
    ...
    echo '<p class="buildpro-post-field"><label>' . esc_html__('Location', 'buildpro') . '</label><input type="text" name="location_project" class="regular-text" value="' . esc_attr($loc) . '" placeholder="' . esc_attr__('Location', 'buildpro') . '"></p>';
    echo '</div>';
}
