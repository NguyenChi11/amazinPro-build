...
function buildpro_project_about_render_meta_box($post)
{
    ...
    echo '<p class="buildpro-post-field"><label>About Image</label><input type="hidden" id="buildpro_project_about_image_id" name="about_image_project" value="' . esc_attr($image_id) . '"> <button type="button" class="button" id="buildpro_project_select_about_image">Select photo</button> <button type="button" class="button" id="buildpro_project_remove_about_image">Remove photo</button></p>';
    echo '<div class="about-image-preview" id="buildpro_project_about_image_preview">' . ($thumb ? '<img src="' . esc_url($thumb) . '">' : '<span style="color:#888">No image selected</span>') . '</div>';
    echo '<script>
    (function(){
        ...
        function setImage(id, url){
            input.value = id || "";
            preview.innerHTML = url ? "<img src=\\"" + url + "\\">" : "<span style=\\"color:#888\\">No image selected</span>";
        }
        if(selectBtn){
